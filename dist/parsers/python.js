...
import { isExternalPythonPath } from "./types.js";
...
export function isGroupAnchor(line) {
    return /^\s*\+ Exception Group Traceback \(most recent call last\):$/.test(line);
}
export function parsePythonChain(lines, start) {
    ...
    for (let k = 1; k < blocks.length; k++) {
        ...
        if (rel)
            t.chained = { relation: rel, trace: current };
        current = t;
    }
    return { trace: current, next: i };
}
function peekChainSeparator(lines, from) {
    let j = from;
    while (j < lines.length && lines[j].trim() === "")
        j++;
    let relation = null;
    if (j < lines.length && PY_CHAIN_CAUSE.test(lines[j].trim()))
        relation = "cause";
    if (j < lines.length && PY_CHAIN_CONTEXT.test(lines[j].trim()))
        relation = "context";
    if (!relation)
        return null;
    j++;
...
function parsePythonBlock(lines, start) {
    ...
    while (i < lines.length) {
        ...
        if (frameM) {
            ...
            if (frameM[3] !== undefined)
                f.symbol = frameM[3].trim();
            frames.push(f);
            i++;
        ...
        if (repM) {
            if (frames.length)
                frames[frames.length - 1].repeated = Number(repM[1]);
            i++;
            continue;
        }
        ...
        i++;
        if (i - start > 10000)
            break; // safety valve on adversarial input
    }
    return {
