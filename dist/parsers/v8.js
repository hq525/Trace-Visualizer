...
import { isExternalJsPath } from "./types.js";
export const V8_HEADER = /^(?:Uncaught )?([A-Za-z_$][\w$]*(?:Error|Exception)(?:\s\[[A-Z_]+\])?|AssertionError|Error): (.*)$/;
export const V8_AT = /^(\s*)(?:at|❯)\s+(.+)$/; // ❯ = vitest's frame marker
const V8_ELISION = /^\s*\.\.\. (\d+) lines matching cause stack trace \.\.\.$/;
export function parseV8Block(lines, start, baseIndent = -1) {
    const headM = lines[start].match(V8_HEADER);
    if (!headM)
        throw new Error(`parseV8Block called on non-header line: ${lines[start]}`);
    const type = headM[1];
    ...
    for (let k = 0; k < rawFrames.length; k++) {
        ...
        if (after && !("__elide" in after) && causeRawFrames) {
            const j = causeRawFrames.findIndex((cf) => cf.rawPath === after.rawPath && cf.line === after.line && cf.column === after.column);
            if (j >= f.__elide) {
