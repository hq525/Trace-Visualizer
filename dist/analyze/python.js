...
export async function analyzePythonSource(file, source) {
    const base = {
        file,
        symbols: [],
        calls: [],
...
function walk(node, ctx, out) {
    switch (node.type) {
        case "decorated_definition": {
            ...
            if (def)
                handleDefinition(def, decorators, ctx, out);
            return;
        }
        ...
        case "import_statement": {
            for (const child of node.namedChildren) {
                if (child.type === "dotted_name") {
                    ...
                    if (mod)
                        out.imports.push({ module: mod, names: [], line: line1(node) });
                }
            }
            return;
        }
        ...
        case "call": {
            ...
            const calleeName = fn?.type === "identifier"
                ? fn.text
                : fn?.type === "attribute"
                    ? (fn.childForFieldName("attribute")?.text ?? null)
                    : null;
            if (calleeName) {
                const site = { calleeName, line: line1(node) };
                const enclosing = ctx.fns[ctx.fns.length - 1];
                if (enclosing)
                    site.enclosing = enclosing;
                out.calls.push(site);
            }
...
function handleDefinition(def, decorators, ctx, out) {
...
function decoratorName(decorator) {
    ...
    if (!expr)
        return null;
    if (expr.type === "call")
        return expr.childForFieldName("function")?.text ?? null;
    return expr.text;
}
