...
export function buildGraph(trace, resolved, analyses, meta) {
...
function coreGraph(trace, slice, analyses, meta, isTop) {
    const nodes = [];
    const edges = [];
    const byId = new Map();
    const spineIds = [];
...
function emitStaticCallEdges(byId, analyses, edges) {
    const fnNode = (file, qualifiedName) => byId.get(hashId(file, qualifiedName));
    const seen = new Set();
    let c = 0;
    for (const [file, analysis] of analyses) {
        for (const call of analysis.calls) {
            if (!call.enclosing)
                continue;
            const fromSyms = analysis.symbols.filter((s) => s.kind === "function" && s.name === call.enclosing);
            if (fromSyms.length !== 1)
                continue;
            ...
            for (const imp of analysis.imports) {
                if (!imp.names.includes(call.calleeName))
                    continue;
                for (const [otherFile, other] of analyses) {
                    if (otherFile === file)
                        continue;
                    if (!moduleMatchesFile(imp.module, otherFile))
                        continue;
...
function hashId(file, qualifiedName) {
