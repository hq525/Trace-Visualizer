...
// column; chained exceptions stack as additional spine rows with labeled
// connectors, newest on top (§5.11). Shared by the UI and the exporters.
...
const CONNECTOR_H = 54;
const LEVEL_BOTTOM = 40;
...
export function layoutGraph(graph: TraceGraph): Layout {
  const levels: { graph: TraceGraph; relation?: "cause" | "context" }[] = [{ graph }];
  for (const c of graph.chained ?? []) levels.push({ graph: c.graph, relation: c.relation });
  ...
  for (const level of levels) {
    if (level.relation) {
      connectors.push({
        x: MARGIN_X,
        y: yOffset + CONNECTOR_H / 2 + 6,
        label: level.relation === "cause" ? "caused by" : "during handling of",
      });
      yOffset += CONNECTOR_H;
    }
    const l = layoutLevel(level.graph, yOffset);
    nodes.push(...l.nodes);
    edges.push(...l.edges);
    overflow.push(...l.overflow);
    width = Math.max(width, l.width);
    yOffset += l.height;
  }

  return { nodes, edges, overflow, connectors, width, height: yOffset + LEVEL_BOTTOM };
}

function layoutLevel(graph: TraceGraph, yTop: number): Omit<Layout, "connectors"> {
  ...
  const spineY =
    yTop + Math.max(HEAD_ROOM, 46 + aboveRows * ROW_STEP + (aboveRows > 0 ? RADIUS_GAP : 0));
  ...
  const height =
    spineY - yTop + NODE_H + (belowRows > 0 ? RADIUS_GAP + belowRows * ROW_STEP : 0) + LEVEL_BOTTOM;
  return {
    nodes,
    edges,
    overflow,
