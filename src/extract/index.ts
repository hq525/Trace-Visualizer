  ...
  for (const rawLine of text.split("\n")) {
    ...
    if (trimmed.startsWith("{") && trimmed.endsWith("}")) {
      ...
      if (obj && typeof obj === "object") {
        let found = false;
        for (const v of walkStrings(obj as Record<string, unknown>)) {
...
function* walkStrings(obj: Record<string, unknown>, depth = 0): Generator<string> {
  if (depth > 3) return;
...
export function extractTraces(text: string): ExtractedTrace[] {
  const lines = cleanInput(text);
  const traces: ExtractedTrace[] = [];
  let i = 0;
